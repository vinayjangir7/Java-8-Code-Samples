...
public class SummarizingDemo {
    ...
    private static Book findCostliestBook() {
        return Book.menu.stream().collect(reducing((d1, d2) -> d1.getPrice() > d2.getPrice() ? d1 : d2)).get();
    }
    ...
    private static Book findCostliestBookUsingComparator() {
        ...
        return Book.menu.stream().collect(reducing(morePriceOf)).get();
    }

    private static int calculateTotalPrice() {
        return Book.menu.stream().collect(summingInt(Book::getPrice));
    }

    private static Double calculateAveragePrice() {
        return Book.menu.stream().collect(averagingInt(Book::getPrice));
    }
    ...
    private static String getShortCatalog() {
        return Book.menu.stream().map(Book::getTitle).collect(joining());
    }
